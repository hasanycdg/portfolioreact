...
const Contact = () => {
    ...
    const handleSubmit=(e)=>{
        e.preventDefault();
        const SERVICE_ID = "service_bqdlgt4";
        const TEMPLATE_ID = "template_womo199";
        const PUBLIC_KEY = "UW3ajgM-RPVJmFRLe";
    ...
    <div className='md:p-0 md:pt-24 md:px-40 bg-[radial-gradient(#e5e7eb_1px,transparent_1px)] [background-size:16px_16px] bg-[white] p-8 md:mt-0 mt-6 pt-16'>
        ...
        <div className='flex justify-center relative lg:-top-8 gap-6 flex-wrap'>
            <div className='p-4 border md:w-80 rounded-lg bg-[radial-gradient(#e5e7eb_1px,transparent_1px)] [background-size:16px_16px] bg-[white] h-fit'>
                <h1 className='md:font-medium md:text-2xl font-medium text-xl'>Direct contact</h1>
                <div className='mt-5 space-y-4 font-poppins text-sm text-slate-700'>
                    <a className='flex items-center gap-3 hover:text-black transition-colors' href={`mailto:${profile.email}`}>
                        <LuMail size={18} />
                        <span>{profile.email}</span>
                    </a>
                    <a className='flex items-center gap-3 hover:text-black transition-colors' href={`tel:${profile.phone.replace(/\s+/g, '')}`}>
                        <LuPhone size={18} />
                    ...
                    <a className='inline-block underline text-slate-800' href={profile.linkedin} target="_blank" rel="noreferrer">
                        LinkedIn Profile
                    </a>
                </div>
            </div>
